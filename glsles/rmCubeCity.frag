...
{
    float dFunc = smin(sphere(pos, 0.408), box(pos, vec3(1.0, 7.000, 1.0)), 1.004);
    float boxGrid = opRep(pos, vec3(1.0,1.0,1.0));
    dFunc = smin(dFunc, boxGrid, 3.576 + cos(u_time));
	return dFunc; 
}
...
void main() {
    ...
    vec3 cameraOrigin = vec3(2.379,-0.801,4.424);
    ...
    const float MAX_DIST = 20.; // Change if object is further than 20 from camera
    ...
    if (dist < EPSILON) 
    {
        ...
        vec3 color = vec3(0.100,0.840,0.727) *diffuse;
