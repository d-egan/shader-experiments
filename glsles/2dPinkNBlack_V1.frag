...
float stroke3(float x, float  a, float v) {
    float d = smoothstep(0.980,1.001,x+v*0.5) - smoothstep(1.480,1.497,x-v*0.5);
...
void main() {
    ...
    float circle = stroke3(r, 1.096, 0.672);
    vec3 color = vec3(0.875,0.523,0.448);
    color *= stroke3(a, 1.740 * sin(u_time) * pos.x, 1.000 * st.y * r);
